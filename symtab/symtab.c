...
struct symtab_stack_s symtab_stack; // pointer to symbol table stack. one stack per shell
int symtab_level;   // current level in stack. 0 or something else
...
void init_symtab(void)
{
    ...
    // allocate memory for and initialise global table
...
// call this whenever new symbol table needed
...
// free memory used by symbol table after use
...
// print contents of local symbol table
...
struct symtab_entry_s *add_to_symtab(char *symbol)
{
    ...
    // do_lookup() to check if entry already exists. else define new one, add to table and then return
    // variable and function names are unique and act as keys
...
// remove table entry which matches given name
...
// search for variable with given name. if variable already exists in table
...
// basically do_lookup() but searches whole stack instead, starting with local symbol table
...
// free memory used to store old entry value. creates copy of new value and stores in symbol table entry
void symtab_entry_setval(struct symtab_entry_s *entry, char *val)
{
    if(entry->val)  // if old value exists
    {
        free(entry->val);
    }
    ...
    {
        char *val2 = malloc(strlen(val)+1); // make new pointers
...
// add new table to stack and assign it as local table
...
// create new empty table and push on top. uses symtab_stack_add
...
// pop table on top (local table)
struct symtab_s *symtab_stack_pop(void)
{
    if(symtab_stack.symtab_count == 0)  // can't pop if no table
    {
        return NULL;
    }
    ...
    symtab_stack.symtab_list[--symtab_stack.symtab_count] = NULL;   // pointer NULL
    symtab_level--;

    // check to modify local and global table variables accordingly
